...
function tokenVapid(origen) {
  ...
  const cue = b64url(
    JSON.stringify({
      aud: origen,
      exp,
      sub: process.env.VAPID_SUBJECT || "mailto:arielmbenz@gmail.com",
    }),
  );
...
/* Igual que enviarPush pero conservando qué contestó el servicio de push.
   Sin esto, un rechazo de Apple y un timeout se ven idénticos desde afuera:
   "error", y a diagnosticar a ciegas. */
export async function enviarPushDetalle(endpoint, urgencia = "normal") {
  let origen;
  try {
    origen = new URL(endpoint).origin;
  } catch {
    return { estado: "muerta", motivo: "endpoint ilegible" };
  }
  try {
    ...
    // El cuerpo de un rechazo suele decir exactamente qué está mal (un sub
    // inválido, una firma que no valida). Se lee siempre y se recorta.
    let cuerpo = "";
    try {
      cuerpo = (await r.text()).slice(0, 300);
    } catch {
      /* sin cuerpo, el código alcanza */
    }
    const estado =
      r.status === 404 || r.status === 410 ? "muerta" : r.ok ? "ok" : "error";
    return { estado, status: r.status, servicio: origen, cuerpo };
  } catch (e) {
    return {
      estado: "error",
      motivo: e.name === "TimeoutError" ? "timeout" : String(e.message || e),
    };
  }
}
...
export async function enviarPush(endpoint, urgencia = "normal") {
  return (await enviarPushDetalle(endpoint, urgencia)).estado;
}
