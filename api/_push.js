// api/_push.js — Web Push sin dependencias.
//
...
function tokenVapid(origen) {
  ...
  const cue = b64url(
    JSON.stringify({ aud: origen, exp, sub: process.env.VAPID_SUBJECT || "mailto:info@example.com" }),
  );
...
export async function redis(...comando) {
  if (!hayAlmacen()) throw new Error("Falta configurar el almacén (KV_REST_API_URL / _TOKEN)");
