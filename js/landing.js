...
async function pintarNivel() {
  ...
  try {
    ...
    /* Los umbrales oficiales los publica la estación del INA y vienen en la
       respuesta. Los de arriba son el respaldo para cuando contesta el
       reporte diario, que no los trae. */
    if (typeof j.alerta === "number") alerta = j.alerta;
    if (typeof j.evacuacion === "number") evacuacion = j.evacuacion;
    real = j.altura;
    sim = real;
